// tests/integration/test_level7_having.cpp — Level 7 HAVING integration tests
...
TEST_F(Level7HavingTest, HavingWithCount) {
    parsers::Parser parser;
    std::string query = "SELECT department, COUNT(*) FROM employees GROUP BY department HAVING COUNT(*) > 1";
...
TEST_F(Level7HavingTest, HavingWithSum) {
    parsers::Parser parser;
    std::string query = "SELECT department, SUM(salary) FROM employees GROUP BY department HAVING SUM(salary) > 150000";
    auto ast = parser.parse_query(query);
    ASSERT_NE(ast, nullptr);
...
TEST_F(Level7HavingTest, HavingWithAvg) {
    parsers::Parser parser;
    std::string query = "SELECT department, AVG(salary) FROM employees GROUP BY department HAVING AVG(salary) > 85000";
    auto ast = parser.parse_query(query);
    ASSERT_NE(ast, nullptr);
...
TEST_F(Level7HavingTest, HavingWithMultipleConditions) {
    parsers::Parser parser;
    std::string query = "SELECT department, COUNT(*), SUM(salary) FROM employees GROUP BY department HAVING COUNT(*) >= 2 AND SUM(salary) > 150000";
    auto ast = parser.parse_query(query);
    ASSERT_NE(ast, nullptr);
...
TEST_F(Level7HavingTest, HavingWithComparisonOperators) {
    parsers::Parser parser;

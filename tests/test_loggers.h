...
TEST_CASE("Logger singleton", "[logger]") {
    auto& logger1 = mnesso::loggers::Logger::get_instance();
    auto& logger2 = mnesso::loggers::Logger::get_instance();
    REQUIRE(&logger1 == &logger2);
}

TEST_CASE("ConsoleTarget emits logs", "[logger]") {
    auto target = mnesso::loggers::ConsoleTarget::create();
    REQUIRE(target != nullptr);
    REQUIRE(target->name() == "Console");
}

TEST_CASE("FileTarget creates file", "[logger]") {
    auto target = mnesso::loggers::FileTarget::create("/tmp/mnemosyne_test.log");
    REQUIRE(target != nullptr);

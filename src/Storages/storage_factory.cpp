...
void StorageFactory::register_engine(
    std::string name, std::function<std::shared_ptr<IStorage>(std::string)> creator) {
...
auto StorageFactory::create(std::string name, std::string engine) -> std::shared_ptr<IStorage> {
    ...
    if (it == registry_.end()) {
        throw common::Exception{"StorageFactory: unknown storage engine: " + engine,
...
struct RegisterBuiltinEngines {
    RegisterBuiltinEngines() {
        ...
        factory.register_engine("Memory", [](std::string name) {
            return storages::MemoryStorage::create(std::move(name));
        });
        factory.register_engine("File", [](std::string name) {
            return storages::FileStorage::create(std::move(name), "");
        });
        factory.register_engine("Dictionary", [](std::string name) {
